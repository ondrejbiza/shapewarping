...
def cpd_transform(source: npt.NDArray, target: npt.NDArray, alpha: float=2.0) -> npt.NDArray:
    ...
    warp = np.dot(reg.W, reg.G)
    return np.hstack(warp)


def warp_gen(canonical_index: int, objects: List[npt.NDArray], scale_factor: float=1., alpha: float=2.0, visualize: bool=False) -> Tuple[List[npt.NDArray], List[float]]:
    """
...
def pick_canonical_simple(points: List[npt.NDArray]) -> int:
    ...
    for i in range(len(points)):
        ...
        for j in range(len(points)):
            if i != j:
                with torch.no_grad():
                    cost = utils.chamfer_distance_batch_pt(points[i][None], points[j][None]).cpu()
...
def pick_canonical_warp(points: List[npt.NDArray], alpha: float=2.0) -> int:
    ...
    cost_sums = []
    for i in range(len(points)):
        _, costs = utils.warp_gen(i, points, alpha=alpha)
        cost_sums.append(np.sum(costs))
    return int(np.argmin(cost_sums))


def pca_fit_transform(distances: npt.NDArray, n_dimensions: int=4) -> Tuple[npt.NDArray, PCA]:
...
def main(args: argparse.Namespace):
    ...
    rotation = Rotation.from_euler("zyx", [args.rot_z, args.rot_y, args.rot_x]).as_matrix()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rot-x", type=float, default=False, help="x-axis rotation in radians.")
    parser.add_argument("--rot-y", type=float, default=False, help="y-axis rotation in radians.")
    parser.add_argument("--rot-z", type=float, default=False, help="z-axis rotation in radians.")

    parser.add_argument("--num-surface-samples", type=int, default=10000, help="Number of points to sample on the surface of each mesh.")
